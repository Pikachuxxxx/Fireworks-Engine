...
namespace fireworks { namespace graphics {
    ...
    public:
        Renderable2D(maths::vec3 position, maths::vec2 size, maths::vec4 color)
            : m_Position(position), m_Size(size), m_Color(color), m_Texture(nullptr), m_Frame(1), m_TotalFrames(1)
        { 
            setUVDefaults();
        }

		Renderable2D(maths::vec3 position, maths::vec2 size, maths::vec4 color, int linearFrameIndex, int totalFrames)
			: m_Position(position), m_Size(size), m_Color(color), m_Texture(nullptr), m_Frame(linearFrameIndex), m_TotalFrames(totalFrames)
		{
        ...
        void setUVFrame() const
        {
            ...
            float frameWidth = texSize.x / m_TotalFrames, frameHeight = texSize.y;
